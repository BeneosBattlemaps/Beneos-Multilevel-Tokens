...
- **Fixed:** Three entries that were still hardcoded English inside `de.json` (`SettingCloneActorLink`, `SettingCloneActorLinkHint`, `FieldClonedTokenOpacity`) are now properly translated. Same fix applied to the other stale files where applicable.
- **Fixed:** `module.json` `languages` array reordered and normalized; `pt.json` replaced by `pt-BR.json` + `pt-PT.json`.
